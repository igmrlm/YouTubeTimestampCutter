...
for i, timestamp in enumerate(range(len(timestamps)), start=1):
    ...
    if end_keyframe is not None:
        ffmpeg_command = f'ffmpeg -i "{video_file}" -ss {start_keyframe} -to {end_keyframe} -vcodec copy -ab 320k "{output_filename}"'
    else:
        ffmpeg_command = f'ffmpeg -i "{video_file}" -ss {start_keyframe} -vcodec copy -ab 320k "{output_filename}"'
