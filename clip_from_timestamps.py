...
def create_keyframe_database(video_file):
    print("Creating keyframe database, this may take a while")
    keyframe_database = {}
...
for i in range(len(timestamps) - 1):
    start_time_str, description = timestamps[i].split(' ', 1)
    end_time_str, _ = timestamps[i + 1].split(' ', 1)
    ...
    # Use ffmpeg to extract the clip with input file name + timestamp description
    output_filename = f"{output_directory}/{input_filename}_{description}.mp4"
